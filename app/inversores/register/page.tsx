      ...
      <div className="min-h-screen flex items-center justify-center px-4 pt-20 pb-10">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-gray-500 text-sm mt-2">Portal exclusivo de inversores</p>
          </div>

          {/* Steps */}
          <div className="flex gap-2 mb-8">
            {[1, 2].map(s => (
              <div key={s} className={`h-1 flex-1 rounded-full transition-all ${s <= step ? 'bg-[#C9A84C]' : 'bg-[#1a2535]'}`} />
            ))}
          </div>
            ...
            {step === 2 && (
              <>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="text-xs text-gray-500 block mb-1">Presupuesto mínimo €</label>
                    <input className="input" type="number" placeholder="50.000"
                      value={form.budget_min} onChange={e => set('budget_min', e.target.value)} />
                  </div>
                  <div>
                    <label className="text-xs text-gray-500 block mb-1">Presupuesto máximo €</label>
                ...
                <div>
                  <label className="text-xs text-gray-500 block mb-1">ROI mínimo esperado %</label>
                ...
                <div>
                  <label className="text-xs text-gray-500 block mb-2">Zonas de interés</label>
                ...
                <div>
                  <label className="text-xs text-gray-500 block mb-1">Plazo de inversión</label>
                ...
                <div className="flex gap-3">
                  ...
                  <button type="submit" disabled={loading}
                    ...
                    {loading ? 'Creando cuenta...' : 'Acceder al Dashboard →'}
                  </button>
                </div>
              </>
            )}
