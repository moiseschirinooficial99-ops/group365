    ...
    try {
      ...
      await fetch('/api/leads', {
        ...
        body: JSON.stringify({
          ...
          preferred_zone: form.preferred_zones[0],
          investment_timeline: form.investment_timeline,
          source: 'inversores',
        }),
      })
      ...
      <main className="bg-[#0F1419] min-h-screen">
        ...
        <div className="min-h-screen flex items-center justify-center px-4 pt-20 pb-10">
          ...
          >
            ...
            <Link href="/inversores/login" className="btn-primary text-sm px-8 py-3 inline-block">
              Ir al login
            </Link>
          </motion.div>
        </div>
      ...
      <div className="min-h-screen flex items-center justify-center px-4 pt-20 pb-10">
        ...
        >
          <div className="text-center mb-7">
            ...
            <p className="text-[#8B96A5] text-sm mt-2">Crear cuenta de inversor</p>
          </div>
          ...
          <div className="flex gap-2 mb-7">
            {[1, 2].map(s => (
              <div
                key={s}
                className={`h-0.5 flex-1 rounded-full transition-all duration-300 ${s <= step ? 'bg-[#C9A84C]' : 'bg-white/10'}`}
              />
            ))}
          </div>
          ...
          <form onSubmit={handleRegister} className="space-y-4">
            {step === 1 && (
              <>
                <div>
                  <label className="text-xs text-[#8B96A5] block mb-1.5">Nombre completo</label>
                ...
                <div>
                  <label className="text-xs text-[#8B96A5] block mb-1.5">Contraseña (mín. 6 caracteres)</label>
            ...
            {step === 2 && (
              <>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="text-xs text-[#8B96A5] block mb-1.5">Presupuesto mín. €</label>
                    <input className="input" type="number" placeholder="50.000"
                      value={form.budget_min} onChange={e => set('budget_min', e.target.value)} />
                  </div>
                  <div>
                    <label className="text-xs text-[#8B96A5] block mb-1.5">Presupuesto máx. €</label>
                ...
                <div>
                  <label className="text-xs text-[#8B96A5] block mb-1.5">ROI mínimo esperado %</label>
                ...
                <div>
                  <label className="text-xs text-[#8B96A5] block mb-1.5">Plazo de inversión</label>
                  <select className="input" value={form.investment_timeline}
                    onChange={e => set('investment_timeline', e.target.value)}>
          ...
          <p className="text-center text-[#8B96A5] text-xs mt-6">
            ¿Ya tienes cuenta?{' '}
            <Link href="/inversores/login" className="text-[#C9A84C] hover:underline">
              Iniciar sesión
            </Link>
          </p>
        </motion.div>
